...
4. ~~**S6–S12 + daily J у графі**; Home-розвилка; web-демо.~~  
   **Зроблено:** локальний журнал однієї події + Home «остання подія / Відкрити звіт» (iOS + web); локальні нагадування I 24/48 (iOS `UNUserNotificationCenter`; web — демо Notification API).  
   **Далі** — валідатор, повний M/ETHANE у чернетці, рецензія draft K3/K4, QR/handover polish.
